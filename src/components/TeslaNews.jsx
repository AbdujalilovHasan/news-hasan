...
class TeslaNews extends Component {
    constructor(props) {
        super(props);
        this.state = {
            teslaArticles: [],
    ...
    fetchData = async () => {
        try {
            const tesla = await axios.get('https://newsapi.org/v2/everything?q=tesla&from=2024-08-21&sortBy=publishedAt&apiKey=12e354c794094034910be3f3b9cb021c');
            this.setState({ teslaArticles: tesla.data.articles });
        } catch (error) {
    ...
    render() {
        const { teslaArticles, searchQuery, currentPage, articlesPerPage } = this.state;
        const filteredArticles = teslaArticles.filter(article =>
        ...
        return (
            <div className="container mt-4">
                ...
                />
                <h1 className="mb-4">Tesla News ({currentArticles.length})</h1>
                <div className="row">
