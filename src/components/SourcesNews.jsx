...
class SourcesNews extends Component {
    ...
    fetchData = async () => {
        try {
            const response = await axios.get('https://newsapi.org/v2/top-headlines?sources=techcrunch&apiKey=12e354c794094034910be3f3b9cb021c');
            this.setState({ 
                SourcesArticles: response.data.articles,
                filteredArticles: response.data.articles // Dastlabki qidiruv natijalarini o'rnatish
            });
        } catch (error) {
    ...
    handleSearchChange = (e) => {
        ...
        const filteredArticles = this.state.SourcesArticles.filter(article =>
            article.title.toLowerCase().includes(query.toLowerCase())
        );

        this.setState({ 
            searchQuery: query, 
            filteredArticles, // Qidiruv natijalarini yangilash
            currentPage: 1 
        });
    };
        ...
        return (
            <div className="container mt-4">
                <input
                    ...
                    placeholder="Search sources News"
                    value={searchQuery}
                    onChange={this.handleSearchChange}
                />
                <h1 className="mb-4">Sources News ({currentArticles.length})</h1>
                <div className="row">
                ...
                <div className="d-flex  gap-2 justify-content-center align-items-center mt-4 mb-5">
